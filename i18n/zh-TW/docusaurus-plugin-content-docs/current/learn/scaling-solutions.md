...
與 Kaia 主鏈相連的服務鏈統稱為服務鏈網絡。請注意，在 Kaia 未來的迭代中，服務鏈和主鏈之間的連接方法可能會發生變化。
...
為了保證數據的完整性，服務鏈可以自動將每個服務鏈塊的哈希值作為特殊事務錨定到主鏈上。這種數據錨定可以向服務用戶確保，服務鏈中的數據一旦創建就不能更改。

### 價值轉移<a id="value-transfer"></a>

為了幫助服務提供商（SP）在不同鏈之間輕鬆遷移服務用戶和價值，可以啟用
在不同鏈之間傳輸代幣，如 KAIA（Kaia 的原生價值單位）和 dApp 發行的 Kaia 代幣。用戶可以通過向一個特殊合約（稱為橋合約）發送交易，輕鬆申請將代幣轉移到其他鏈上。
