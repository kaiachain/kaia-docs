...
**discport** 用於檢查已知鄰居是否是可到達的 kaia 節點，並獲取其鄰居地址以建立新連接。請注意，這是一個 UDP 端口。默認情況下，UDP 端口或 "discport "與 TCP 端口使用相同的端口。如果節點使用不同的 `discport` 端口，可通過 `discport` 查詢參數指定。

以下兩個 URL 顯示了一個節點的 KNI 示例，該節點的 IP 地址為 "10.0.0.1"，TCP 監聽端口為 "32323 "和 "32324"。如果省略了 `discport`，它將被設置為 UDP 端口 `32323`，與 `port` 的值相同。
