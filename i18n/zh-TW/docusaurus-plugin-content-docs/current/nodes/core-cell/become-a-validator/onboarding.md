...
目前單一押注合約可以註冊多個管理帳戶，但從 \*\* 無授權階段 2\*\* 開始，每個押注合約只允許註冊一個管理帳戶。 因此，我們建議註冊一個 Safe Wallet 帳戶作為管理員。
...
5. 輸入釣魚合約管理位址和多重登入閾值。 若您將 Safe Wallet 的地址設為 **管理員地址**，並將門檻設定為 `1`，則多簽名功能將由 Safe 負責處理。 如果您輸入多個管理位址，並將臨界值設定為任意值，多重登入功能將在 **Manage Staking** 功能表中處理。
