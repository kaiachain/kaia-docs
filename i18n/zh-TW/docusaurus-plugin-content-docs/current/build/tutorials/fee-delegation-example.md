...
When `fee payer` receives the `senderTxHashRLP`, `fee payer` signs the `senderTxHashRLP` again with their private key and sends the transaction to Kaia. 下面的代碼片段說明瞭這一過程。
