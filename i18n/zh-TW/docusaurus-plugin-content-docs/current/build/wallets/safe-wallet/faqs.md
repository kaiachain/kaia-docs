...
## 轉用 Safe Global 會影響我現有的 Safe 服務嗎？ <a id="Does moving to Safe Global affect my existing Safe"></a>

不。您的「保險箱」是一個鏈上智慧帳戶。 Safe Global 是針對相同合約所設計的另一種 **UI**。您的 Safe 地址、持有者、門檻及資產均維持不變。您無需重新建立「保險箱」，也無需轉移資金。

詳情請參閱 **[遷移至 Safe Global](./migrate-to-safe-global.md)**。
...
您的通訊錄和暱稱原本是儲存於已停用的 `safe.kaia.io` 介面中（而非鏈上），因此並未被轉移過來——請在 Safe Wallet 的 **通訊錄** 中重新輸入這些資料。這僅會影響標籤，不會影響鏈上的所有權或餘額。請參閱 [遷移至「安全全球」](./migrate-to-safe-global.md#address-book-labels)。
...
若連線後未顯示「Safe」，請前往 [app.safe.global/welcome/accounts](https://app.safe.global/welcome/accounts) 並點擊 **管理清單**。
