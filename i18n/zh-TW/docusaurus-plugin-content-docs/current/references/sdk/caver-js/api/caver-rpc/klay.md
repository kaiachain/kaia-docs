    ...
    transactions：[
        {
            ...
            transactionHash: '0x109d2836d9fde9d8081a27dd6ac545fd7a53530a56bdc40f2a11e5d6dbc2a09f',
            transactionIndex：'0x0',
            類型：'TxTypeFeeDelegatedSmartContractExecution',
            typeInt：49,
            value: '0x0',
        },
    ],
...
| from               | 字符串     | 發送人地址。                                                                                                                                    |
| gas                | 字符串     | 發送人提供的gas。                                                                                                                                |
| gasPrice           | 字符串     | 由發件人提供用 peb 的 Gas Price                                                                                                                   |
| 哈希                 | 字符串     | 交易的哈希值。                                                                                                                                   |
...
// 當事務處於待處理狀態時，將返回 `blockHash`、`blockNumber` 和 `trasnactionIndex` 的默認值。
...
| 有效GasPrice         | 字符串     | 從發送方扣除的每筆gas的實際價值。 在 Magma 硬分叉之前，該值等於交易的天然氣價格。 Magma 硬分叉後，它等於區塊頭中的 "baseFee "值。                                                                                                                                                                                                                                                                                                                                                                                                          |
| 付費者                | 字符串     | (可選）付費人地址。                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| feePayerSignatures | 數組      | (可選）付費人簽名對象數組。 簽名對象包含三個字段（V、R 和 S）。 V 包含 ECDSA 恢復 ID。 R 包含 ECDSA 簽名 r，而 S 包含 ECDSA 簽名 s。                                                                                                                                                                                                                                                                                                                                                                              |
