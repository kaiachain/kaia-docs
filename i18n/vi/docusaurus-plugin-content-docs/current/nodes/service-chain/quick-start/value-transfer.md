...
Tạo tài khoản thử nghiệm bằng [Kaia Toolkit](https://toolkit.kaia.io/account/) và nhận KAIA thử nghiệm từ [vòi](https://faucet.kaia.io/). Then send some KAIA to the `parentOperator`. `childOperator` has to get KAIA from the test account generated by `homi` ([Refer to EN Setup and SCN Connection Guide](en-scn-connection.md)).
...
- Thay thế `parent.key` bằng khóa riêng của tài khoản thử nghiệm được tạo bằng [Kaia Toolkit](https://toolkit.kaia.io/account/) ở bước trước.
