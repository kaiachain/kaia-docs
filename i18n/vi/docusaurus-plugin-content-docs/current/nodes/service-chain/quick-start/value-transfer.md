...
Create a test account on a [Kairos Wallet](https://baobab.wallet.klaytn.foundation/) and get test KAIA from the faucet. Then send some KAIA to the `parentOperator`. `childOperator` has to get KAIA from the test account generated by `homi` ([Refer to EN Setup and SCN Connection Guide](en-scn-connection.md)).
...
- Replace `parent.key` with the private key of the test account created from [Kairos Wallet](https://baobab.wallet.klaytn.foundation/) in the previous step.
