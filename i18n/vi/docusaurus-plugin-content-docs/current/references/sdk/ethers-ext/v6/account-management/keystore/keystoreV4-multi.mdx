...
<CH.Spotlight>
  <CH.Code>
    ...
    // Kaia V4 với nhiều khóa dựa trên vai trò.https://toolkit.klaytn.foundation/misc/generateKeystore
    const encryptedKey = `{
      "version": 4,
    ...
    // Đảm bảo rằng đối tượng Keystore v4 đã giải mã không thể được mã hóa lại thành Keystore v4. 
    // Vì vậy, ví dụ này chỉ hiển thị việc giải mã Keystore v4 và mã hóa nó thành Keystore v3.
    // Đảm bảo rằng địa chỉ Kaia bị mất khi mỗi tài khoản của Keystore v4 được mã hóa thành Keystore v3,
    // vì Keystore v3 không hỗ trợ hệ thống tài khoản Kaia.
    async function main() {
