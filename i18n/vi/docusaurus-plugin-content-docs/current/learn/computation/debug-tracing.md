...
Debug tracing is a feature that extracts additional information while executing a transaction in a VM. Đúng như tên gọi, công cụ này chủ yếu được sử dụng để gỡ lỗi giao dịch hoặc sự cố chi tiêu gas. Since VMs are deterministic, once a transaction is confirmed, the same trace will be produced no matter when it is traced. In other words, you can debug a confirmed transaction after the fact, making it a useful tool for dApp development.
...
- Đối với các giao dịch đảo ngược, cả hai trường đều được cung cấp.
