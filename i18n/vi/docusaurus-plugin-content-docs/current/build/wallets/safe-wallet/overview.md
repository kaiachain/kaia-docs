...
Đối với người dùng trước đây sử dụng Safe do Kaia lưu trữ (`safe.kaia.io`), vui lòng tham khảo **[Chuyển sang Safe Global](./migrate-to-safe-global.md)**: các tài khoản Safe hiện có vẫn còn hiệu lực; chỉ có giao diện người dùng được khuyến nghị là thay đổi.
...
Khi sử dụng [API Kit](./safe-wallet-api-kit.md) hoặc các công cụ khác của Safe SDK, hãy truyền ID chuỗi Kaia chính xác. Các điểm cuối dịch vụ có thể thay đổi khi `safe.kaia.io` ngừng hoạt động; vui lòng tham khảo tài liệu về Dịch vụ Giao dịch Toàn cầu Safe để biết các chuỗi được hỗ trợ và thông tin cấu hình.
