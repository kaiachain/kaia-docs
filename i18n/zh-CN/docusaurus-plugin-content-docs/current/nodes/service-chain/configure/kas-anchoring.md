...
如设计部分所述，您可以将服务链数据锚定到 Kaia 主链上。本页介绍如何通过 [KAS (Kaia API Service)](https://www.klaytnapi.com) 启用数据锚定。

一旦开启，服务链中的节点就可以定期将其链数据（区块数据）锚定到 Mainnet 或 Kairos，以证明服务链的存在和不变性。这确保了服务链的安全性和可信度。
...
首先，您需要在 [KAS 控制台网站](https://www.klaytnapi.com) 上注册 KAS，获得 KAS 账户。请访问上述网站并在 KAS 中注册。
...
登录后，您可以创建如下凭证。访问密钥 ID "和 "秘密访问密钥 "或 "授权 "将用于调用 KAS API。
...
然后，您可以像下面这样查看操作员列表。请注意，设置服务链节点需要运营商的地址。

![create operator](/img/nodes/kas-operator-list-en.png)

## 配置服务链节点<a id="configure-service-chain-node"></a>

获得 API 证书、Anchor API 信息（API 端点和参数）以及 KAS 中的操作员账户后，就可以设置服务链节点了。您需要编辑服务链节点的配置文件（`kscnd.conf`, `kspnd.conf`, `ksend.conf`），如下所示。
...
现在你可以走了。您可以运行服务链节点。您将看到与 KAS 锚点 API 相关的日志信息，如下所示。
