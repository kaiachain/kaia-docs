...
<CH.Spotlight>
  <CH.Code>
    ...
    // Kaia V4 具有多个基于角色的密钥。 https://toolkit.klaytn.foundation/misc/generateKeystore
    const encryptedKey = `{
      "version"：4,
    ...
    // 确定解密后的密钥存储 v4 对象不支持再次加密密钥存储 v4。 
    // 因此，本示例仅演示了解密 Keystore v4 并将其加密为 keysotre v3。
    //
    // 因为 keystore v3 不支持 kaia 帐户系统，所以在将 keystore v4 的每个帐户加密为 keysotre v3 时，要确保 kaia 地址丢失。
    async function main() {
  ...
  从 **@kaiachain/ethers-ext** 软件包中导入钱包类。
  ...
  **使用**当前密码**解密**密钥库
  ...
  您可以使用**加密密钥**和**新密码**生成**新密钥存储**。 请注意，新的加密密钥将是 keystore v3。
