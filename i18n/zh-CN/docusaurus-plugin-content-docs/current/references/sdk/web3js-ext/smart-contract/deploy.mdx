...
定义合同 Abi，可以从编译后的 solidity 代码中获取
...
使用 `Web3.eth.Contract`创建合同实例，并定义 **contractAbi**
...
创建交易对象，其中 `data` 是编码后的合约字节码，`from: senderAddress` 是发送方的钱包地址
...
使用 `signTransaction` 在**发送人**的账户上签署交易
