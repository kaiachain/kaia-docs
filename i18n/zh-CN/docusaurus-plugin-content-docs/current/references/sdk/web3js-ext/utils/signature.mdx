...
<CH.Spotlight>
  ...
  从 **@kaiachain/web3js-ext** 包中导入 **getSignatureTuple**
  ...
  要将公钥从`{r, s, v}`格式压缩为元组格式，请使用 `getSignatureTuple` 并将**signature object**作为参数
  ...
  要将公钥从紧凑的 65 字节格式压缩成元组，请使用 `getSignatureTuple` 并将**compact signature**作为参数
