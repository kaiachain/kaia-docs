...
<CH.Spotlight>
  ...
  从 "账户 "对象中，您可以使用 "address_from_private_key "获取**公共地址**，或使用 "compressed_key "获取**压缩公共密钥**。
  ...
  根据未压缩公钥的 `x` 和 `y` 坐标，可以使用 `compressed_key_from_xy` 和 `compressed_key_and_address_from_xy` 获取公钥和地址。
