...
发送**alue transfer tx**。 It **will** be in the **tx pool** and **canceled** before execution. 如果取消 tx 已执行，则会失败。
...
创建一个**TxType.CANCEL**类型的空事务。 您可以使用 **empty_tx** 工具来获取已填写默认字段的 tx。
...
使 **nonce** 与之前发送的 **value transfer tx** 相同。 您需要设置要取消的**相同的 tx nonce**。
