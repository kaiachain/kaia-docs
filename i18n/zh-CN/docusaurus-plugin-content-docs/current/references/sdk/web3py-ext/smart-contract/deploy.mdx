...
这些是编译成字节码并将部署到 Kairos 测试网络的**合同**
...
设置合约 **bytebode** 和 **abi**。 使用 remix 或 solc 编译器编译后，您可以获得字节码和 abi
...
发送 tx，部署智能合约。 这将自动签署并发送包含构造函数调用数据的 tx
