...
您的通讯录和昵称存储在已停用的 `safe.kaia.io` 接口中（本地存储），而非链上，因此并未被迁移过来——请在 Safe Wallet 的 **通讯录** 中重新输入这些信息。这仅影响标签，不影响链上的所有权或余额。请参阅 [迁移到“安全全球”](./migrate-to-safe-global.md#address-book-labels)。
...
如果连接后未显示“Safe”，请访问 [app.safe.global/welcome/accounts](https://app.safe.global/welcome/accounts) 并点击 **管理列表**。
