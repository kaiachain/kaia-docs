...
TxTypeFeeDelegatedValueTransferWithRatio 用于用户发送 KAIA。 由于 Kaia 提供了多种交易类型，使每种交易类型只服务于一个目的，因此 TxTypeFeeDelegatedValueTransferWithRatio 只限于将 KAIA 发送到外部拥有的账户。 因此，TxTypeFeeDelegatedValueTransferWithRatio 只有在 "to "为外部所有账户时才被接受。 要将 KAIA 转移到智能合约账户，请使用 [TxTypeFeeDelegatedSmartContractExecutionWithRatio](#txtypefeedelegatedsmartcontractexecutionwithratio)。 该交易类型将进行以下更改。

1. 付费者的余额按交易费的给定比例减少。
2. 汇款人的余额会因剩余的交易费而减少。 例如，如果 `feeRatio` 为 30，则 30% 的费用将由付款人支付，其余 70% 的费用将由发件人支付。
3. 发送方的 nonce 增加一个。
...
### 寄件人签名的 RLP 编码<a id="rlp-encoding-for-signature-of-the-sender"></a>
...
### 缴费人签名的 RLP 编码<a id="rlp-encoding-for-signature-of-the-fee-payer"></a>
...
TxTypeFeeDelegatedValueTransferMemoWithRatio 用于用户发送带有特定信息的 KAIA。 TxTypeFeeDelegatedValueTransferMemoWithRatio 仅在 `to` 为外部所有账户时才被接受。 要将 KAIA 转移到智能合约账户，请使用 [TxTypeFeeDelegatedSmartContractExecutionWithRatio](#txtypefeedelegatedsmartcontractexecutionwithratio)。 该交易类型将进行以下更改。

1. 费用支付者的余额按交易费金额的费用比率减少。
2. 汇款人的余额会因剩余的交易费而减少。 例如，如果 "feeRatio "为 30，则 30% 的费用将由付款人支付，其余 70% 的费用将由发件人支付。
3. 发送方的 nonce 增加一个。
4. `value` KAIA 由发送方传送给接收方。
...
TxTypeFeeDelegatedSmartContractDeployWithRatio 部署智能合约。 交易费的给定比例由缴费人支付。 该交易类型将进行以下更改。

1. 费用支付者的余额按交易费金额的费用比率减少。
2. 汇款人的余额会因剩余的交易费而减少。 例如，如果 `feeRatio` 为 30，则 30% 的费用将由付款人支付，其余 70% 的费用将由发件人支付。
3. 发送方的 nonce 增加一个。
4. 智能合约与 `input` 中的代码一起部署。 部署地址将通过收据中的 `contractAddress` 返回。
5. `value` KAIA 由发送方传送给接收方。

### 属性<a id="attributes"></a>

| 属性                 | 类型                                                                                                                                                                          | 说明                                                                                                                                                                                                                |
| :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| type               | uint8 \(Go\)                                                                                                                                           | TxTypeFeeDelegatedSmartContractDeployWithRatio 的类型。 必须是 0x2a。                                                                                                                                                     |
| nonce              | uint64 \(Go\)                                                                                                                                          | 用于唯一标识发件人交易的值。 如果一个发送方生成了两个具有相同 nonce 的交易，则只执行其中一个。                                                                                                                                                               |
| gasPrice           | \*big.Int \(Go\)                                                                                                                       | 以 `kei` 为单位的气体单价，发件人将支付交易费。 交易费的计算公式为 `gas`\* `gasPrice`。 例如，如果交易消耗了 10 单位天然气，而 gasPrice 为 10^18，则交易费为 10 KAIA。 见 [Unit of KAIA](../../learn/token-economics/kaia-native-token.md#units-of-kaia). |
| gas                | uint64 \(Go\)                                                                                                                                          | 交易允许使用的最大燃气量。                                                                                                                                                                                                     |
...
TxTypeFeeDelegatedSmartContractExecution 使用 "输入 "中的给定数据执行智能合约。 只有当 `to` 是智能合约账户时，才接受 TxTypeFeeDelegatedSmartContractExecutionWithRatio。 要将 KAIA 转至外部所有的账户，请使用 [TxTypeFeeDelegatedValueTransferWithRatio](#txtypefeedelegatedvaluetransferwithratio)。 该交易类型将进行以下更改。

1. 如果 `to` 是智能合约账户，则根据 `input` 执行代码。 否则，此交易将被拒绝。
2. 费用支付者的余额按交易费金额的费用比率减少。
3. 汇款人的余额会因剩余的交易费而减少。 例如，如果 `feeRatio` 为 30，则 30% 的费用将由付款人支付，其余 70% 的费用将由发件人支付。
4. 发送方的 nonce 增加一个。
5. 如果提供了 "value"，"value "KAIA 将从发送方转移到 "to "智能合约。 合同应具有接收 KAIA 的可支付后备功能。

### 属性<a id="attributes"></a>

| 属性                 | 类型                                                                                                                                                                          | 说明                                                                                                                                                                                                                |
| :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| type               | uint8 \(Go\)                                                                                                                                           | TxTypeFeeDelegatedSmartContractExecutionWithRatio 的类型。 必须为 0x32。                                                                                                                                                  |
| nonce              | uint64 \(Go\)                                                                                                                                          | 用于唯一标识发件人交易的值。 如果一个发送方生成了两个具有相同 nonce 的交易，则只执行其中一个。                                                                                                                                                               |
| gasPrice           | \*big.Int \(Go\)                                                                                                                       | 以 `kei` 为单位的气体单价，发件人将支付交易费。 交易费的计算公式为 `gas`\* `gasPrice`。 例如，如果交易消耗了 10 单位天然气，而 gasPrice 为 10^18，则交易费为 10 KAIA。 见 [Unit of KAIA](../../learn/token-economics/kaia-native-token.md#units-of-kaia). |
| gas                | uint64 \(Go\)                                                                                                                                          | 交易允许使用的最大燃气量。                                                                                                                                                                                                     |
...
TxTypeFeeDelegatedAccountUpdateWithRatio 更新给定账户的密钥。 交易费的给定比例由缴费人支付。 该交易类型将进行以下更改。

1. 费用支付者的余额按交易费金额的费用比率减少。
2. 汇款人的余额会因剩余的交易费而减少。 例如，如果 `feeRatio` 为 30，则 30% 的费用将由付款人支付，其余 70% 的费用将由发件人支付。
3. 发送方的 nonce 增加一个。
4. 账户密钥用 `key` 更新。
5. 交易执行后，从该账户发出的交易将使用此 "密钥 "进行验证。

### 属性<a id="attributes"></a>

| 属性                 | 类型                                                                                                                                                                          | 说明                                                                                                                                                                                                                  |
| :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| type               | uint8 \(Go\)                                                                                                                                           | TxTypeFeeDelegatedAccountUpdateWithRatio 的类型。 必须为 0x22。                                                                                                                                                             |
| nonce              | uint64 \(Go\)                                                                                                                                          | 用于唯一标识发件人交易的值。 如果一个发送方生成了两个具有相同 nonce 的交易，则只执行其中一个。                                                                                                                                                                 |
...
| from               | common.Address \(Go\)                                                                                                                  | 发件人地址。 详见 [签名验证](./transactions.md#signature-validation)。                                                                                                                                                           |
| key                | AccountKey \(Go\)                                                                                                                                      | [AccountKey](../accounts.md#account-key)将更新到账户。                                                                                                                                                                     |
| feeRatio           | uint8 \(Go\)                                                                                                                                           | 收费者的收费比率。 有效范围在 1 到 99 之间。 Zero\(0\) 是不允许的。 100 及以上也不允许。                                                                                                                                       |
| txSignatures       | \[\]\{\*big.Int, \*big.Int, \*big.Int\} \(Go\) | 发件人签名。 详见 [签名验证](./transactions.md#signature-validation)。                                                                                                                                                           |
| feePayer           | common.Address \(Go\)                                                                                                                  | 缴费人地址。                                                                                                                                                                                                              |
| feePayerSignatures | \[\]\{\*big.Int, \*big.Int, \*big.Int\} \(Go\) | 缴费人签名。                                                                                                                                                                                                              |
...
TxTypeFeeDelegatedCancelWithRatio 取消执行事务池中具有相同 nonce 的事务。 更多详情，请参阅 [TxTypeCancel](./basic.md#txtypecancel)。

以下更改将适用于该交易类型。 1. 缴费人的余额按交易费金额的既定费用比率减少。 2. 汇款人的余额会因剩余的交易费而减少。 3. 发送方的 nonce 增加一个。

### 属性<a id="attributes"></a>

| 属性                 | 说明                                                                                                                                                                          | 类型                                                                                                                                                                                                                |
| :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| type               | uint8 \(Go\)                                                                                                                                           | TxTypeFeeDelegatedCancelWithRatio 的类型。 必须是 0x3a。                                                                                                                                                                  |
| nonce              | uint64 \(Go\)                                                                                                                                          | 用于唯一标识发件人交易的值。 如果一个发送方生成了两个具有相同 nonce 的交易，则只执行其中一个。                                                                                                                                                               |
| gasPrice           | \*big.Int \(Go\)                                                                                                                       | 以 `kei` 为单位的气体单价，发件人将支付交易费。 交易费的计算公式为 `gas`\* `gasPrice`。 例如，如果交易消耗了 10 单位天然气，而 gasPrice 为 10^18，则交易费为 10 KAIA。 见 [Unit of KAIA](../../learn/token-economics/kaia-native-token.md#units-of-kaia). |
| gas                | uint64 \(Go\)                                                                                                                                          | 交易允许使用的最高交易费金额。                                                                                                                                                                                                   |
| from               | common.Address \(Go\)                                                                                                                  | 发件人地址。 详见 [签名验证](./transactions.md#signature-validation)。                                                                                                                                                         |
...
这笔交易不会改变 Kaia 区块链的状态，只是发送者的非ce 增加了一个。

### 属性<a id="attributes"></a>

| 属性                 | 类型                                                                                                                                                                          | 说明                                                                                                                                                                                                                |
| :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| type               | uint8 \(Go\)                                                                                                                                           | TxTypeFeeDelegatedChainDataAnchoringWithRatio 的类型。 必须是 0x4a。                                                                                                                                                      |
| nonce              | uint64 \(Go\)                                                                                                                                          | 用于唯一标识发件人交易的值。 如果一个发送方生成了两个具有相同 nonce 的交易，则只执行其中一个。                                                                                                                                                               |
| gasPrice           | \*big.Int \(Go\)                                                                                                                       | 以 `kei` 为单位的气体单价，发件人将支付交易费。 交易费的计算公式为 `gas`\* `gasPrice`。 例如，如果交易消耗了 10 单位天然气，而 gasPrice 为 10^18，则交易费为 10 KAIA。 见 [Unit of KAIA](../../learn/token-economics/kaia-native-token.md#units-of-kaia). |
| gas                | uint64 \(Go\)                                                                                                                                          | 交易允许使用的最高交易费金额。                                                                                                                                                                                                   |
| from               | common.Address \(Go\)                                                                                                                  | 发件人地址。 详见 [签名验证](./transactions.md#signature-validation)。                                                                                                                                                         |
...
PublicKey.X 0x3a514176466fa815ed481ffad09110a2d344f6c9b78c1d14afc351c3a51be33d
PublicKey.Y 
SigHash 0xd79dbb964bee2d3807e214a247141a1fcb066a67de99e90750aac4a2a0b776de
Signature0xf845f84326a0c612a243bcb3b98958e9cce1a0bc0e170291b33a7f0dbfae4b36dafb5806797da00c734423492ecc21cc53238147c359676fcec43fcc2a0e021d87bb1da49f0abf
FeePayerPrivateKey 0xb9d5558443585bca6f225b935950e3f6e69f9da8a5809a83f51c3365dff53936
FeePayerPublicKey.X 0x327434d4cfc66ef8857d431419e9deebdc53a3e415edcc55382e2d417b8dd102
FeePayerPublicKey.Y 
SigHashFeePayer 
SenderTxHash 0xa0670c01fe39feb2d2442adf7df1957ade3c5abcde778fb5edf99c80c06aa53c
  ...
  类型：          TxTypeFeeDelegatedChainDataAnchoringWithRatio
  From：          0xa94f5374Fce5edBC8E2a8697C15331677e6EbF0B
  Nonce: 18
  GasPrice: 0x5d21dba00
  ...
  十六进制           
```
