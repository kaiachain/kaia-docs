...
이 튜토리얼에서는 Kaia에서 수수료 위임 가치 전송 트랜잭션이 어떻게 작동하는지 보여드리기 위해 Kaia SDK(ethers-ext)를 사용하여 간단한 서버-클라이언트 예제를 만드는 과정을 안내합니다. 튜토리얼과 예제 코드는 Kairos 테스트넷에서 테스트되었습니다.
...
수수료 납부자`가 `발신자TxHashRLP`를 받으면, `수수료 납부자`는 개인 키로 `발신자TxHashRLP\`에 다시 서명하고 트랜잭션을 Kaia로 보냅니다. 아래 코드 스니펫은 이 과정을 설명합니다.
...
수수료 납부자가 발신자를 대신하여 Kaia에 트랜잭션을 제출할 때, `senderTxHashRLP` 유형은 `FeeDelegatedValueTransfer` 유형의 트랜잭션이어야 한다는 점에 유의하세요.
...
:::note
Kaia체인/ethers-ext@^1.2.0은 노드 22 이상을 권장합니다.
:::
...
이제 수신한 `senderTxHashRLP`를 `feePayerPrivateKey`로 서명하고 이를 Kairos 테스트넷으로 전송하는 수수료 납부자 서버인 `feepayer_server.js`를 작성해 보겠습니다.
...
들어오는 `data`가 있으면 `data`에 `feePayerPrivateKey`로 서명하고 이를 Kaia 블록체인으로 보냅니다. 데이터`가 `sender_client.js`의 `senderTxHashRLP\`라고 가정합니다.
...
`Sender` 개인 키로 트랜잭션에 서명하고 서명된 트랜잭션을 수수료 위임 서비스(즉, 수수료 납부자 서버)로 전송합니다. 그런 다음 '수수료 납부자' 주소, 'Tx 해시'를 포함한 수수료 대리인 서비스의 응답을 받게 됩니다. 'Tx 해시'는 Kaia 네트워크에 제출된 트랜잭션의 해시입니다.
...
### 4.4 KaiaScan에서 보기 <a href="#4-4-kaiascan" id="4-4-kaiascan"></a>

위의 거래는 [KaiaScan](https://kairos.kaiascan.io/tx/0x7cb1e8d20b4db7d9db1abc094781e1af83a9391153aab8cc935510639a548222?tabId=overview&page=1)에서도 확인할 수 있습니다.
