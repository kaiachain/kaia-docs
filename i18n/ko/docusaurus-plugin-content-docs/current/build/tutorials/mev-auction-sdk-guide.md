...
:::warning 서비스 공지 - MEV 경매 인프라가 일시적으로 일시 중지됨

현재 사용 패턴에 맞춘 운영 최적화의 일환으로, 메인넷과 카이로스 모두에서 **Auctioneer** 및 **MEV Explorer**를 포함한 MEV 경매 인프라가 일시적으로 일시 중지되었습니다. 경매인 엔드포인트(`auctioneer.kaia.io`, `auctioneer-kairos.kaia.io`) 및 MEV 탐색기 엔드포인트(`mev.kaia.io`, `mev-kairos.kaia.io`)는 현재 사용할 수 없습니다.

이 일시 중지로 인해 카이아의 MEV 프레임워크나 검색자 생태계가 종료되는 것은 아닙니다. CN 레벨 MEV 기능은 **활성화**된 상태로 유지됩니다. 인프라는 생태계 상황과 검색자 수요에 따라 MEV 경매 운영을 재개할 필요가 있을 때 다시 온라인 상태로 전환됩니다. 재개 날짜가 설정되지 않았습니다.
