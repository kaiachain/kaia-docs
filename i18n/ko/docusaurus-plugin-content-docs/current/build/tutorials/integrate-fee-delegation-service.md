...
2. \*\*논스가 올바르게 관리되는지 확인: \*\*거래가 올바른 논스로 제출되고 새로운 거래가 진행되기 전에 실패하거나 오래된 논스가 올바르게 처리되는지 확인합니다. 대량 거래 시나리오에서 논스 관리에 대한 자세한 전략은 [안정적인 거래를 위한 논스 관리 방법](../cookbooks/how-to-manage-nonce.md)을 참조하세요.
...
\*\*Q: 논스 오류가 계속 발생하거나 트랜잭션이 멈췄습니다. 카이아에서 논스 문제를 해결하려면 어떻게 해야 하나요?
**A:** 논스 갭이나 중복으로 인해 거래가 차단될 수 있습니다. 먼저 계정의 온체인 트랜잭션 수를 확인한 다음, 다음 논스를 이에 맞춰 조정하세요. 대량 또는 수수료 위임 흐름의 경우 계정별 잠금 기능이 있는 오프체인 논스 저장소를 사용하고, 동일한 논스를 사용하여 중단된 트랜잭션은 새 논스를 전송하는 대신 더 높은 수수료로 대체하세요. 전체 패턴과 복구 단계는 [안정적인 거래를 위한 논스 관리 방법](../cookbooks/how-to-manage-nonce.md)을 참조하세요.