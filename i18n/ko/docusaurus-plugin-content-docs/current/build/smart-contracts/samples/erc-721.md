...
이 튜토리얼은 [Kaia 토큰 표준](../token-standard.md), 특히 [대체 불가능한 토큰 표준(ERC-721)](../token-standard.md#non-fungible-token-standard-kip-17)을 준수하는 ERC-721 호환 토큰 예제를 만들 수 있도록 도와줍니다.
...
개발자는 위 인터페이스를 기반으로 새로운 기능과 로직을 추가하여 토큰을 커스터마이징하고 Kaia 네트워크에 배포할 수 있습니다.
...
- Please visit [Kaia Plugin for Remix](https://ide.kaia.io) and create a `MyERC721Card` contract. 전체 소스코드는 [ERC-721 스마트 컨트랙트 작성하기](#1-writing-erc-721-smart-contract)에서 확인하실 수 있습니다.
- 컨트랙트를 배포할 계정을 생성합니다.
