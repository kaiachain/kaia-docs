...
스마트 컨트랙트를 실행하고 싶을 때 파이페이에 **결제 요청**을 포함하는 \*\*"트랜잭션"\*\*을 할 수 있습니다.
...
<CH.Spotlight>
  ...
  발신자 계정, 수신자 주소, 전송할 값(이 예시에서는 0 KLAY), 트랜잭션 유형 \*\*(TxType.SmartContractExecution)\*\*을 지정하여 **prepareTransactionRequest**를 사용하여 **스마트 컨트랙트 작성**을 위한 트랜잭션 요청을 생성합니다.
  ...
  발신자는 `klay_sendRawTransaction`을 사용하여 트랜잭션을 Kaia 블록체인에 제출하고, 그 결과(일반적으로 트랜잭션 해시)가 기록됩니다. 발신자가 거래 수수료를 지불합니다.
