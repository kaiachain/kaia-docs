...
<CH.Spotlight>
  ...
  HTTP\*\* 전송(`http()`)을 통해 **카이아 카이로스 테스트넷**(`카이로스`)과 상호 작용하도록 구성된 **createPublicClient**를 사용하여 **publicClient**를 생성합니다. 퍼블릭 클라이언트는 개인 키나 서명 기능 없이 **블록체인 쿼리** 또는 **스마트 컨트랙트 상태 읽기**와 같은 **읽기 전용 작업**에 사용됩니다.
