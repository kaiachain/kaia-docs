...
ブロックチェーンプラットフォームにおけるトランザクションとは、ブロックチェーンの状態を変更するノード間で送信されるメッセージのことである。 Klaytn also redesigns its transaction model. トランザクションはそれぞれの目的に応じて様々なタイプに分けられ、パフォーマンスの最適化の可能性を見つけ、再設計されたアカウントモデルをサポートする。 Detailed information about the transaction model can be found [here](../../build/transactions/transactions.md).
