...
| gasPrice           | \*big.Int (Go)                                                                                                               | A unit price of gas in `peb` the sender will pay for a transaction fee. 取引手数料の金額は、`gas` \* `gasPrice` として計算されます。 For example, if the transaction consumes 10 units of gas and gasPrice is 10^18, the transaction fee will be 10 KLAY. KAIAのユニット】(../../learn/token-economics/kaia-native-token.md#units-of-kaia)を参照。 |
