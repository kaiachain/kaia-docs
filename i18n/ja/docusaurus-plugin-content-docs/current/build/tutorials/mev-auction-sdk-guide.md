...
:::warning サービス開始のお知らせ - MEVオークションインフラ一時停止のお知らせ

現在の利用パターンに合わせた運用の最適化の一環として、**オークショニア**と**MEVエクスプローラー**を含むMEVオークションインフラは、**メインネットとカイロス**の両方で一時的に停止しています。 オークショニアエンドポイント（`auctioneer.kaia.io`、`auctioneer-kairos.kaia.io`）と MEV Explorer エンドポイント（`mev.kaia.io`、`mev-kairos.kaia.io`）は現在利用できない。

この一時停止は、カイアのMEVフレームワークやサーチャーのエコシステムを終了させるものではない。 CN レベルの MEV 機能は **有効** のままです。 このインフラは、生態系の状況や検索者の需要により、MEVオークションの活動を再開する必要が生じた場合にオンラインに戻される。 再開時期は未定。
