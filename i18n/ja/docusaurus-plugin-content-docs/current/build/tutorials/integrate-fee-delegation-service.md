...
2. \*\*トランザクションが正しいnonceで提出され、新しいトランザクションを進める前に、失敗したnonceや古いnonceが適切に処理されることを保証する。 バルク・トランザクション・シナリオにおけるnonce管理の詳細な戦略については、[How to Manage Nonces for Reliable Transactions](../cookbooks/how-to-manage-nonce.md) を参照のこと。
...
\*\*Q: nonceエラーが発生したり、トランザクションがスタックしたりします。
**A:** Nonceのギャップや重複はトランザクションをブロックする可能性があります。 まず自分のアカウントのオンチェーン取引数を確認し、次のnonceをそれに合わせる。 一括フローや料金委譲フローでは、アカウントごとにロックするオフチェーン・ノンス・ストアを使用し、同じノンスを使用して行き詰まったトランザクションを、新しいものを送信するのではなく、高い料金で置き換える。 パターンとリカバリーのステップの完全なセットについては、[How to Manage Nonces for Reliable Transactions](../cookbooks/how-to-manage-nonce.md) を参照のこと。