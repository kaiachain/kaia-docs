...
\*\*ステップ1エクスプローラーペインでコントラクトフォルダーを選択し、新規ファイルボタンをクリックして`SBT.sol`という名前の新規ファイルを作成します。
...
Now that we have our Hardhat project set up let’s fork the Klaytn Mainnet using Hardhat.  ターミナルを開き、次のコマンドを実行する。
