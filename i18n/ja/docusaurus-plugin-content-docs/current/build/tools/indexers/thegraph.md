...
The GraphにおけるKaiaのサポートは、**2026年8月31日**をもって終了します。 その日付を過ぎると、Kaiaサブグラフのインデックス作成は停止し、それらのクエリエンドポイントはデータを返さなくなります。

お使いのDappがThe Graph上のKaiaサブグラフからデータを取得している場合は、\*\*[The Graphからの移行](./migrate-from-the-graph.md)\*\*の手順に従い、サポート終了日までに[Goldsky](./goldsky.md)、[SubQuery](./subquery.md)、またはセルフホスト型のGraphノードへ移行してください。 サブグラフのコードは変更なしでそのまま引き継がれます。

以下のガイドは、本サービスが稼働している間、参考として掲載されています。
...
The Graphでは、**2026年8月31日**まで、Kaiaのメインネットおよびテストネットの両方がサポートされます。
