...
以前KaiaがホストしていたSafe（`safe.kaia.io`）をご利用だったユーザーの方は、\*\*[Safe Globalへの移行](./migrate-to-safe-global.md)\*\*をご覧ください。既存のSafeは引き続き有効です。変更されるのは、推奨されるUIのみです。
...
[API Kit](./safe-wallet-api-kit.md) やその他の Safe SDK ツールを使用する際は、正しい Kaia チェーン ID を指定してください。 `safe.kaia.io` の提供終了に伴い、サービスエンドポイントが変更される可能性があります。サポートされているチェーンや設定については、Safe Global Transaction Service のドキュメントをご参照ください。
