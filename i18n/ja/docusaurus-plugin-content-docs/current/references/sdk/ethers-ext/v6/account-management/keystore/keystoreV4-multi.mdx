...
この例では、**keystore V4 multi**を暗号化および復号化する方法を示します。
...
<CH.Spotlight>
  <CH.Code>
    ...
    // Kaia V4 with multiple role-based keys. https://toolkit.klaytn.foundation/misc/generateKeystore
    const encryptedKey = `{
      "version": 4,
    ...
    // Be sure that kaia address is lost when each account of keystore v4 is encrypted as keysotre v3,
    // because keystore v3 does not support kaia account system.
    async function main() {
  ...
  **@kaiachain/ethers-ext** パッケージから Wallet クラスをインポートします。
  ...
  キーストアの現在の**パスワード**と**新しいパスワード**を指定する。
  ...
  **暗号化キー**と**新しいパスワード**を使用して、**新しいキーストア**を生成できます。 暗号化された新しい鍵はkeystore v3となる。
