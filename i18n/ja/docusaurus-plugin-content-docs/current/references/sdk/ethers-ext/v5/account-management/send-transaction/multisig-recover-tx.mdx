...
送信者の**アドレス**、**秘密鍵**、その他の**重み付きマルチシグ**秘密鍵を定義する。
...
TxType.ValueTransfer`で値転送トランザクションを作成する：TxType.ValueTransfer`で値転送トランザクションを作成し、後で `klay_recoverFromTransaction` で復元できるようにする。
