...
type.ValueTransfer`で**値転送**トランザクションを作成する：TxType.ValueTransfer`でトランザクションを作成し、後で `klay_recoverFromTransaction` で復旧できるようにする。
...
klay_recoverFromMessage\`を使用して署名されたトランザクションからアドレスを回復する。
