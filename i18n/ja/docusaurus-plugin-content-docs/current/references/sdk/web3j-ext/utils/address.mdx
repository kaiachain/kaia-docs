...
<CH.Spotlight>
  ...
  圧縮された公開鍵から `{x, y}` 形式の `AccountPublicKey` に変換するには、`AccountKeyPublicUtils.decompressKey` を使用します。
  ...
  `AccountKeyPublic`を`圧縮公開鍵`文字列に圧縮するには、`AccountKeyPublicUtils.toCompressedPublicKey`を使用する。
