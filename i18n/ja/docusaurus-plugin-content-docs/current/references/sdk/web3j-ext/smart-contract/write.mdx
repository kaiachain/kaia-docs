...
スマートコントラクトの状態を更新する必要がある場合、スマートコントラクトと**「トランザクション」**を行うことができる。
...
import org.web3j.protocol.kaia.Web3j; import org.web3j.example.keySamplekaia.Web3j;
...
public class WriteContractExample {
    ...
    public static void run() throws Exception {
        ...
        String contractAddr = "0x95Be48607498109030592C08aDC9577c7C2d505";
        ...
        // 契約書込み前の数値を取得
        System.out.println("書込み前の数値 : " + counter.数値().send());

        // 契約書込み（インクリメント関数）
        counter.increment().send();

        // 契約書込み後の数値を取得
        System.out.println("書込み後の数値 : " + counter.数値().send());
    }.
