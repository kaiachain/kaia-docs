...
def web3_account_update_role_based():
    ...
    account_update_tx = merge(account_update_tx, {
        'from' : to_checksum_address("0x5bdf2b3c21564c023a4a735935a2b7a238c4ccea"),
        'key' : {
...
秘密鍵から**トランザクション**鍵をロードし、それを**トランザクション・ロール・キー**として使用する。 トランザクションの署名に使用できる
...
秘密鍵から**更新**鍵をロードし、それを**更新ロール鍵**として使用する。 この口座の口座更新取引に署名するために使用できます。 つまり、このキーを持っている人だけが、このアカウントを更新できるということだ。
