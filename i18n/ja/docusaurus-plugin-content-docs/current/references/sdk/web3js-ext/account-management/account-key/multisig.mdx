...
変更する送信者アドレス**、送信者秘密鍵**および新しい**muti-sig秘密鍵**を定義する。
...
秘密鍵**と**プロバイダー\*\*を使って3つの送信者のウォレットを作成する。
...
計算された**公開鍵**とその**重み**を持つ**keys**フィールドを持つトランザクショ ンを宣言し、**type**を**WeightedMultiSig**と指定する。
...
kaiaネットワークにトランザクションを送信し、ブロックチェーン上でトランザクションが完了した場合、txレシートを返す。
