...
setNumber」関数のデータをエンコードし、データをバイトに変換する。
...
params `type：TxType.SmartContractExecution`, `from: senderAddr`, `to: contractAddr`, `data: data` が必要である。
