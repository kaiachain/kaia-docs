...
* Deploy Kaia contracts to the Kaia Network (Baobab) using Foundry Scripting System.
...
In this section, you will deploy the generated code from klaytn contracts wizard to the Kaia Testnet Baobab using Foundry. The generated code will serve as a starting point and customized further to fit an airdrop contract for KIP7 and KIP17 tokens.  While on the other end the generated code for KIP37 will be used as it is. 
...
Having generated the contract code for all our Kaia contracts, the next step is to deploy to the Kaia Testnet Baobab using Foundry solidity scripts.
...
We’re going to deploy our generated smart contract to the Kaia Baobab Testnet, but to do this we’ll need to configure Foundry a bit, by setting things like a Baobab RPC URL, the private key of an account that’s funded with test KLAY. 
