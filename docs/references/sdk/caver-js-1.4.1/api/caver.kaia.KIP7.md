...
The `caver.kaia.KIP7` helps you easily handle a smart contract that implements KIP-7 as a JavaScript object on the Kaia blockchain.

The `caver.kaia.KIP7` inherits [caver.kaia.Contract](caver.kaia.Contract.md) to implement the KIP-7 token contract. The `caver.kaia.KIP7` holds the same properties of `caver.kaia.Contract` whereas additional methods to implement extra features. This section only introduces the newly added bound methods of the `caver.kaia.KIP7`.

The abi and bytecode used in the caver.kaia.KIP7 were implemented using the example of [openzeppelin](https://github.com/OpenZeppelin/openzeppelin-contracts/tree/master/contracts/token/ERC20).

For more information about KIP-7, see [Kaia Improvement Proposals](https://kips.klaytn.foundation/KIPs/kip-7).

**NOTE** `caver.kaia.KIP7` is supported since caver-js [v1.4.0](https://www.npmjs.com/package/caver-js/v/1.4.0).

## caver.kaia.KIP7.deploy <a id="caver-kaia-kip7-deploy"></a>

```javascript
caver.kaia.KIP7.deploy(tokenInfo, deployer)
```

Deploys the KIP-7 token contract to the Kaia blockchain. A contract deployed using caver.kaia.KIP7.deploy is a fungible token that follows the KIP-7 standard.
...
| deployer | String | The address of the account to deploy the KIP-7 token contract. This account must have enough KAIA to deploy. |
...
```javascript
new caver.kaia.KIP7([tokenAddress])
```
...
> const kip7Instance = new caver.kaia.KIP7()

// Create a KIP7 instance with a token address
> const kip7Instance = new caver.kaia.KIP7('0x{address in hex}')
```
...
```javascript
> const kip7Instance = new caver.kaia.KIP7(address)
...
`Promise` returns `Object` - The receipt containing the result of the transaction execution. If you want to know about the properties inside the receipt object, see the description of [getTransactionReceipt](./caver.kaia/transaction/transaction.md#gettransactionreceipt). Receipts from KIP17 instances have an 'events' attribute parsed via ABI instead of a 'logs' attribute.
