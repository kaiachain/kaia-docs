...
## Release Notes for Kaia v1.5.0

### New features <a id="new-features"></a>
- State migration (https://github.com/klaytn/klaytn/pull/507)
    - New JSON-RPC API to remove the unnecessary and old state/storage trie, saving the storage space for Kaia node (Cypress) by 75% (800 GB -> 200 GB)
...
- Warm-up trie node cache (https://github.com/klaytn/klaytn/pull/548)
    - New JSON-RPC APIs to speed up the block processing by iterating the latest state trie and caching it
        - debug.startWarmUp()
        - debug.stopWarmUp()
    - Until now, Kaia node had to continuously access a database for processing a block due to uncached state trie nodes, which results in the delay in block processing. The introduced "warm-up" feature reduces this delay by iterating the last state trie and caching it.
