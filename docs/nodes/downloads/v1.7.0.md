...
Kaia v1.7.0 contains important changes for the protocol upgrade which may result in incompatible changes like Ethereum Hard-forks.
...
- Moves addresses of Kaia precompiled contracts (#912)
  - vmLog: 0x09 → 0x3fd
  - feePayer: 0x0a → 0x3fe
...
- Introduces a minimum staking amount for validators as a governance item (#920 #963 #1023 #1024 #1034)
  - If a validator doesn’t stake enough KAIA, the node will not be selected as a proposer or a committee member.

- Randomly selects new committee members on every round change (#997)

### New features
- Introduces two kaia APIs that return a block header: klay_getHeaderByNumber and klay_getHeaderByHash (#975)
- Introduces two istanbul APIs that return demoted validators (#974)
  ...
  - A demoted validator is one of the validators but cannot be a proposer or committee member due to insufficiently staking KAIA.
...
- Added SetHeader method to Kaia client (#1020)
